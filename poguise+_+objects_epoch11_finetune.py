...
FINE_TUNE_SOURCE_CKPT = (
    "/mnt/local-scratch/poguise_data/checkpoints/"
    "actor_object_residual_validated45_20260615_023719/"
    "epoch_checkpoints/epoch=011.ckpt"
)
if not Path(FINE_TUNE_SOURCE_CKPT).is_file():
    raise RuntimeError(f"Fine-tune source checkpoint is missing: {FINE_TUNE_SOURCE_CKPT}")
print("Fine-tune source checkpoint:", FINE_TUNE_SOURCE_CKPT, flush=True)

TS = datetime.now().strftime("%Y%m%d_%H%M%S")
RUN_NAME = f"actor_object_residual_epoch11_finetune10_{TS}"
EPOCH_DIR = str(Path(DATA_DIR) / "checkpoints" / RUN_NAME / "epoch_checkpoints")

cmd = [
    sys.executable, "-u", "train.py",

    "--model_file", FINE_TUNE_SOURCE_CKPT,
    ...
    "--actor_object_residual_max_relation_scale", "0.75",
    ...
    "--poguiseplus_interaction_heatmap_weight", "1.25",
    "--poguiseplus_heatmap_log_eps", "1e-6",
    "--poguiseplus_normalized_heatmap_loss", "1",
    "--poguiseplus_heatmap_mse_scale", "1000",
    "--poguiseplus_interaction_heatmap_pos_loss_weight", "0.08",
    "--poguiseplus_interaction_heatmap_pos_weight", "8.0",
    "--poguiseplus_interaction_heatmap_center_loss_weight", "0.04",
    "--poguiseplus_interaction_heatmap_center_temperature", "10.0",

    "--motion_aux_loss_weight", "0.35",
    "--object_residual_prompt_relation_loss_weight", "0.12",
    "--object_residual_prompt_relation_loss_final_weight", "0.08",
    "--object_residual_relation_loss_decay_start_epoch", "0",
    "--object_residual_relation_loss_decay_end_epoch", "3",
