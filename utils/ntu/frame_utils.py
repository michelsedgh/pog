...
def spatial_sampling(
    ...
    if spatial_idx == -1:
        frames, _, keypoints, obj_keypoints = transform_loc.random_short_side_scale_jitter(
            images=frames,
            min_size=min_scale,
            max_size=max_scale,
        ...
        if random_horizontal_flip:
            frames, _, keypoints, obj_keypoints = transform_loc.horizontal_flip(
                0.5, frames, keypoints=keypoints, obj_keypoints=obj_keypoints
            )
    else:
        ...
        frames, _, keypoints, obj_keypoints = transform_loc.uniform_crop(
            frames,
            crop_size,
            spatial_idx,
            keypoints=keypoints,
...
def spatial_sampling_2crops(
    ...
    assert spatial_idx in [-1, 0, 1, 2]
    if spatial_idx == -1:
        frames, _ = transform_loc.random_short_side_scale_jitter(
            images=frames,
            min_size=min_scale,
            max_size=max_scale,
            inverse_uniform_sampling=inverse_uniform_sampling,
        )
        frames, _ = transform_loc.random_crop(frames, crop_size)
        if random_horizontal_flip:
            frames, _ = transform_loc.horizontal_flip(0.5, frames)
    else:
        # The testing is deterministic and no jitter should be performed.
        # min_scale, max_scale, and crop_size are expect to be the same.
        # assert len({min_scale, max_scale, crop_size}) == 1
        ...
        frames, _ = transform_loc.uniform_crop_2crops(frames, crop_size, spatial_idx)
    return frames
