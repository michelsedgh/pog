#!/usr/bin/env python3
"""Training-only launcher for the detector-guided PO-GUISE+ transfer sweep.

Run this after the Colab/setup cell has written /content/poguise_colab_env.sh.
It intentionally does not clone repos, install packages, authenticate Drive, or
mount data. It only runs the four object-relation diagnostic trainings.
"""
...
TRANSFER4_SWEEP = [
    (
        ...
        "Range test: if object/interaction patches can matter, this should make retention metrics jump.",
        {
            ...
            "--actor_object_confuser_engagement_loss_weight": "1.00",
            "--actor_object_confuser_action_loss_weight": "0.20",
            ...
            "--objectful_low_motion_aug_prob": "0.60",
        },
    ),
    (
    ...
    (
        ...
        "Range test: if relation updates are too weak, this should make object-drop action deltas jump.",
        {
            ...
            "--actor_object_engagement_loss_weight": "0.60",
            "--actor_object_confuser_engagement_loss_weight": "1.25",
            "--actor_object_confuser_action_loss_weight": "0.25",
            "--actor_object_confuser_margin": "0.50",
    ...
    (
        ...
        "Destructive proof: if this wins validation but fails live, object pressure is overriding visual/actor semantics.",
        {
            ...
            "--actor_object_engagement_loss_weight": "1.20",
            "--actor_object_confuser_engagement_loss_weight": "2.00",
            "--actor_object_confuser_action_loss_weight": "0.35",
            "--actor_object_confuser_margin": "0.50",
            ...
            "--objectful_low_motion_aug_prob": "0.80",
        },
    ),
]


def load_colab_env(path: Path) -> dict[str, str]:
    if not path.is_file():
        raise RuntimeError(f"Missing setup env file: {path}. Run the setup cell first.")
    env = {}
    ...
    if missing:
        raise RuntimeError(f"Setup env file is missing: {', '.join(missing)}")
    return env


def run_checked(cmd: list[str], label: str, cwd: Path) -> None:
    print("\n" + "=" * 100, flush=True)
...
def latest_metrics_path(run_dir: Path) -> Path | None:
    paths = sorted(run_dir.glob("version_*/metrics.csv"), key=lambda p: p.stat().st_mtime)
    return paths[-1] if paths else None


def completed_validation_epoch_count(metrics_path: Path | None) -> int:
...
def summarize_run(repo_dir: Path, run_dir: Path, verbose: bool = False) -> None:
...
def drain_training_output(proc: subprocess.Popen[str]) -> None:
    while True:
...
def set_cmd_arg(cmd: list[str], key: str, value: str | int | float) -> None:
    value = str(value)
    if key in cmd:
        idx = cmd.index(key)
        if idx + 1 >= len(cmd):
...
def run_training_with_epoch_summaries(
    repo_dir: Path,
    data_dir: Path,
    cmd: list[str],
    run_name: str,
    ...
    run_dir = data_dir / "checkpoints" / run_name
    ...
    while True:
        ...
        if completed_count > last_completed_val_count:
            ...
            print("#" * 100, flush=True)
            summarize_run(repo_dir, run_dir, verbose=False)
    ...
    summarize_run(repo_dir, run_dir, verbose=True)
    return str(run_dir)


def build_working_cell_base_cmd(env: dict[str, str], run_name: str, epoch_dir: Path) -> list[str]:
    ...
    return [
        ...
        env["FRAME_COUNT_CACHE"],
        "--toyota_object_cache_dir",
        f"{data_dir}/toyota_preprocessed_cache/objects",
        "--toyota_landmark_cache_dir",
        f"{data_dir}/toyota_preprocessed_cache/landmarks",
        "--toyota_split_source",
        "auto",
        "--toyota_val_fraction",
        "0.15",
        "--toyota_test_fraction",
        "0.0",
        "--num_classes",
        "26",
        "--n_landmarks",
        ...
        "1.60",
        "--num_scene_object_tokens",
        "32",
        "--num_object_classes",
        "19",
        "--object_detector_cache",
        env["OBJECT_DETECTOR_CACHE"],
        ...
        "max",
        "--checkpoint_filename",
        "{epoch:03d}-{val_deploy_score:.4f}-{val_f1:.4f}-{val_acc_macro:.4f}",
        "--save_top_k",
        "5",
        "--save_every_epoch_checkpoints",
        "1",
...
def main() -> None:
    ...
    parser.add_argument("--env-file", default="/content/poguise_colab_env.sh")
    args = parser.parse_args()
    if args.epochs <= 0:
        raise SystemExit("--epochs must be positive")

    env = load_colab_env(Path(args.env_file))
    ...
    run_dirs = []
    print("\nTRANSFER4 SWEEP", flush=True)
    print(f"epochs per run: {args.epochs}", flush=True)
    for preset_name, description, overrides in TRANSFER4_SWEEP:
        run_name = f"actor_object_{preset_name}_{ts}"
        epoch_dir = data_dir / "checkpoints" / run_name / "epoch_checkpoints"
        run_cmd = build_working_cell_base_cmd(env, run_name, epoch_dir)
        set_cmd_arg(run_cmd, "--model_name", run_name)
        set_cmd_arg(run_cmd, "--epoch_checkpoint_dir", str(epoch_dir))
        set_cmd_arg(run_cmd, "--max_epochs", str(args.epochs))
        set_cmd_arg(run_cmd, "--t_max_scheduler", str(args.epochs))
        set_cmd_arg(run_cmd, "--save_top_k", "2")
        set_cmd_arg(run_cmd, "--save_every_epoch_checkpoints", "0")
        for key, value in overrides.items():
            set_cmd_arg(run_cmd, key, value)
        print("\n" + "=" * 100, flush=True)
        ...
        print("=" * 100, flush=True)
        run_dirs.append(
            run_training_with_epoch_summaries(
                repo_dir,
                data_dir,
                run_cmd,
                run_name,
