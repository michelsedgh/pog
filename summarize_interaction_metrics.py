...
def compute_interaction_score(df):
    ...
    center = (
        df_metric(df, "val_interaction_heatmap_center_l2", 0.0)
        if df_has_metric(df, "val_interaction_heatmap_center_l2")
        else 0.0
    )
...
def print_decision(epoch_df):
    ...
    if pd.notna(cf_logit):
        if cf_logit > 0.02 and pd.notna(selection_acc) and selection_acc > 0.20:
            print(
                "GOOD SIGN: selected object is being chosen and affects the "
                "true action logit."
            )
        elif cf_logit <= 0.0:
            print(
